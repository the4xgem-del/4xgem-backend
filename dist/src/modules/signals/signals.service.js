...
exports.getUserActiveTier = getUserActiveTier;
exports.serializeSignal = serializeSignal;
const prisma_1 = require("../../lib/prisma");
const redis_1 = require("../../lib/redis");
const planTier_1 = require("../../utils/planTier");
const ApiError_1 = require("../../utils/ApiError");
const marketData_1 = require("../../lib/marketData");
async function getUserActiveTier(userId) {
    if (!userId)
        return "FREE";
...
function serializeSignal(signal, unlocked) {
    const base = {
        id: signal.id,
        pair: signal.pair,
        name: signal.name,
...
exports.signalsService = {
    async list(query, userId) {
        ...
        if (cached) {
            ...
            signals = await prisma_1.prisma.tradingSignal.findMany({
                where: {
            ...
            await (0, redis_1.cacheSet)(cacheKey, signals, 30); // short TTL — signals update frequently
        }
        const total = await prisma_1.prisma.tradingSignal.count({
            where: {
                ...(query.category ? { category: query.category } : {}),
                ...(query.status ? { status: query.status } : {}),
            },
        ...
        return {
            items: signals.map((s) => serializeSignal(s, (0, planTier_1.tierSatisfies)(userTier, s.requiredTier))),
            pagination: { page: query.page, pageSize: query.pageSize, total, totalPages: Math.ceil(total / query.pageSize) },
        };
    },
    async getById(id, userId) {
        const signal = await prisma_1.prisma.tradingSignal.findUnique({ where: { id } });
        if (!signal)
            throw ApiError_1.ApiError.notFound("Signal not found.");
        const userTier = await getUserActiveTier(userId);
        return serializeSignal(signal, (0, planTier_1.tierSatisfies)(userTier, signal.requiredTier));
    },
    async create(authorId, input) {
        const signal = await prisma_1.prisma.tradingSignal.create({
            data: { ...input, authorId },
        });
     ...
     */
    async applyPriceUpdate(displayPair, currentPrice) {
        const candidates = await prisma_1.prisma.tradingSignal.findMany({
        ...
        for (const signal of candidates) {
            ...
            let nextStatus = null;
            const hit = (level) => (isLong ? currentPrice >= level : currentPrice <= level);
            ...
            if (nextStatus && nextStatus !== signal.status) {
                const pipsMoved = (0, marketData_1.computeFloatingPips)(signal.pair, signal.direction, entry, currentPrice) ?? 0;
                const isFinal = nextStatus === "HIT_SL" || nextStatus === "HIT_TP3";
                const result = await prisma_1.prisma.tradingSignal.update({
                    where: { id: signal.id },
                    data: {
                        status: nextStatus,
